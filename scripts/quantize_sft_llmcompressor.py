#!/usr/bin/env python3
"""Quantize the SFT merged Qwen3.6-35B-A3B model to W4A16 AutoRound on CPU.

Run on patricks-mint:
    cd /data/projects/club-3090
    source .venv-quant/bin/activate
    HF_TOKEN=<token> CUDA_VISIBLE_DEVICES="" \
      python3 scripts/quantize_sft_llmcompressor.py
"""
...
def parse_args():
    ...
    parser.add_argument("--output_dir", default="/data/projects/club-3090/models-cache/qwen36-35b-a3b-sft-w4a16-autoround")
...
def main():
    ...
    # Force CPU-only execution; the full bf16 model does not fit on a 24 GB 3090.
    ...
    print("Starting AutoRound W4A16 quantization (CPU, this will take several hours)...")
    oneshot(
        model=model,
        dataset=dataset,
        recipe=recipe,
