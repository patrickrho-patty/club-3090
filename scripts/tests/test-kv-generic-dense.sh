...
#
# Asserts:
#   1. tools/kv-calc.py --calibration still reports the exact line "Overall: 22/22 (100%)"
...
# --- 1. calibration regression contract: exact 22/22 line --------------------
CALIB_OUT="$(tools/kv-calc.py --calibration 2>&1)"
if ! grep -qxF "Overall: 22/22 (100%)" <<<"$CALIB_OUT"; then
  echo "FAIL: --calibration did not report the exact line 'Overall: 22/22 (100%)'" >&2
  echo "----- last 6 lines of --calibration output -----" >&2
  tail -6 <<<"$CALIB_OUT" >&2
  exit 1
fi
echo "PASS: --calibration reports 'Overall: 22/22 (100%)' (curated branches unchanged)"
